package io.dm.model.activities.bosses;
...
import static io.dm.model.activities.donatorzone.DonatorBossPortal.available;
import static io.dm.model.activities.donatorzone.DonatorBossPortal.hasKillsLeft;

public class Nechryarch extends NPCCombat {
    ...
    private static final int RANGED_ANIM = 6368;
    ...
    private static final int MAGIC_ANIM = 6368;

    private static final int SWITCH_ANIM = 6368;

    private static final int MELEE_PRAYER = 7412;
    private static final int RANGED_PRAYER = 7414;
    private static final int MAGIC_PRAYER = 7415;
    ...
    public void init() {
        ...
        npc.deathEndListener = (entity, killer, killHit) -> {
            ...
            if (killer != null && killer.player != null) {
                killer.player.timesKilledDonatorBoss++;
                if (!hasKillsLeft(killer.player)) {
                    killer.player.sendMessage("You have used up all your donator boss kills for today!");
                    killer.player.lastTimeKilledDonatorBoss = System.currentTimeMillis();
                }
            }
        };
        npc.attackNpcListener = (player, npc1, message) -> {
            if (!player.debug && (!available(player.lastTimeKilledDonatorBoss) || !hasKillsLeft(player))) {
                player.sendMessage("You have already killed the donator boss for the max times today.");
                return false;
            }
            return true;
        };
